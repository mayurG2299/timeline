...
function Tweet({ id, userName, userImageUrl, tweetText, tweetImageUrl, likes, isLiked }) {
  // console.log({ likes, isLiked, id });
  const [likesCount, setLikeCount] = useState(isLiked ? JSON.stringify(parseInt(likes) + 1) : likes)
  ...
  return (
    <div className="tweetContainer">
      ...
      <div className="dataContainer">
        ...
        {/* image */}
        {tweetImageUrl ? <img className="tweetImage" src={tweetImageUrl} alt="" /> : null}
        {/* options */}
        <div className="optionsContainer">
          <div className="likeOption">
            ...
            {/* </Tooltip> */}
            {likesCount != "0" ? (
              <h3 className={liked ? "likeCount" : "likedCount"}>{likesCount}</h3>
            ) : null}
          </div>
        </div>
      </div>
